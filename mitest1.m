function I = mitest1(x,y,tf,SCB1,SB1,SN1,SRB1,int2,Sib1,Ref2,rgb2,nptc2,xlimit,ylimit)
tx = x;
ty = y;
r =  tf;
tz=0;%t(3,1);
...
%end
H_xy = -sum(sum(Hc.*(log2(Hc+(Hc==0)))))-sum(sum(Hn.*(log2(Hn+(Hn==0)))))-sum(sum(Hr.*(log2(Hr+(Hr==0)))))-sum(sum(Hi.*(log2(Hi+(Hi==0)))))-sum(sum(Hz.*(log2(Hz+(Hz==0)))));
