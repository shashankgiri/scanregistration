function I = miexp2(y,x,SB1,SN1,SRB1,Sib1,SCB1,nptc2,Ref2,int2,rgb2,xlimit,ylimit)
tx = x(1,1);
ty = y;
r =  x(3,1);%tf;
tz=0;%t(3,1);
...
% [~,SN2] = grid_normal(nptc22.Location,nptc22.Normal,xlimit,ylimit);
% [~,SRB2] = grid_reflectivity(nptc22.Location,Ref2,xlimit,ylimit);
% [~,Sib2] = grid_intensity(nptc22.Location,int2,xlimit,ylimit);
[~,SCB2] = grid_rgb(nptc22.Location,rgb2,xlimit,ylimit);
SB1 = histeq(SB1);
SB2 = histeq(SB2);
% SN1 = histeq(SN1);
% SN2 = histeq(SN2);
...
%end
H_xy = -sum(sum(Hz.*(log2(Hz+(Hz==0)))))-sum(sum(Hc.*(log2(Hc+(Hc==0)))));%-sum(sum(Hn.*(log2(Hn+(Hn==0)))))-sum(sum(Hr.*(log2(Hr+(Hr==0)))))-sum(sum(Hi.*(log2(Hi+(Hi==0)))));
I = -((Hxz + Hyz + Hxc + Hyc)/H_xy);
