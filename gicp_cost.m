function c = gicp_cost( frame, model, varargin)%[tform,TT,TR,mse_profile,c,fun]
...
p = inputParser;
p.addParameter('MaxIterations', 100, @(x)isnumeric(x));
...
if frame.Count > 10000 || model.Count > 10000
    fprintf('It is probably not wise to attempt ICP with %dx%d points',frame.Count, model.Count)
end
...
%     frame.computenormals();     
    ...
    for i=1:min(frame.Count,model.Count)%size(B,2)
        Rmu = [model.Normal(i,:)' [0 1 0]' [0 0 1]'];
        Rnu = [frame.Normal(i,:)' [0 1 0]' [0 0 1]'];
        Cb(:,:,i) = Rmu * C * Rmu';
        Ca(:,:,i) = Rnu * C * Rnu';
    end
...
iter=0;
c=zeros(60,1);
for y=0.01:0.01:6%for iter = 1:maxiter
 iter=iter+1;       
    qt(5:7)=[x,y,-0.0122];
  ...
  last_mse = mse;
        ...
        %[qt, fun(iter,1)] = fminsearch(@cost, qt , struct('Display', 'final', 'TolFun', msethresh*100, 'TolX',0.1));
...
% q_r = quat2rotm([qt(1,1) qt(1,2) qt(1,3) qt(1,4)]);
% q_t=[-qt(1,5) -qt(1,6) -qt(1,7) 1];
% q=[q_r,[0 0 0]';q_t];
% TT = q_t(1,1:3);
% TR = rotm2eul(q_r);
% tform = affine3d(q);
