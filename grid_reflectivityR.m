function [ScanImage,ScanBlur]=grid_reflectivityR(npts,reflectivity,xlimit,ylimit)
 x_min= -ceil(xlimit/2);
 y_min= -ceil(ylimit/2);
no_of_points=length(reflectivity);
...
for i = 1:no_of_points
    ...
    y_loc = ceil((npts(i,2)-y_min)/0.5-y_size/3);
     ...
     grid{x_loc,y_loc}=[grid{x_loc,y_loc};reflectivity(i)];
       end
end
for j=1:size(grid,1)
    for k=1:size(grid,2)
